...
function new_stat_bar(base_sysdir, params)
	...
	if (type(min_value) == "string") then min_value = util.fread_num(base_sysdir.."/"..min_value) end
	if (type(max_value) == "string") then max_value = util.fread_num(base_sysdir.."/"..max_value) end
...
function register_sysmon(base_sysdir)
	local monitor = function ()
		local data = fread_num(base_sysdir)
	end
